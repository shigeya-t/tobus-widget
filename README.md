...
- 都バス運行情報サービスの車両接近情報にもとづく状態表示（実車接近中は「約N分後」、
  実車情報が無い時間帯は定型文で表示）
- 本日の残り定刻（静的時刻表）をあわせて表示。本日分が終わったら翌日の始発に切り替え
...
- **実車情報が無い/定型メッセージのみ**: 次のような定型文（バケット）だけが返る（`td.stopNotes`）
  ...
  | `本日は運休日です。` | 本日は運休 |
  | `運休が発生する場合があります`（備考テロップ） | 運休の可能性として注記（運休日にはしない） |
  | （文言なし・バス表示が空） | 接近中のバスなし |

どちらのレイアウトになるかは系統・時間帯によって変わるため、両方に対応しています
（`Shared/TobusPageParser.swift` の `approachInfo(inTable:)`）。

最後の「文言なし」は、**案内は行われているがまだ接近中のバスが1台もいない**状態です。
tobus.jp が案内自体を停止している「ご案内できません」とは意味が違うため、区別して表示しています。

同じ系統に複数台が接近していることもあります（表のセルは停留所に近い順＝到着が早い順に並びます）。
その場合は1台目を主表示にし、「次 13分後」のように後続も添えます
（小ウィジェットは1台分、中ウィジェットとメニューバーは全台）。

<table>
<tr>
<td align="center"><img src="assets/widget-03.png" width="150"><br><sub>まもなく到着</sub></td>
</tr>
</table>

### 定刻（時刻表）について

車両接近情報ページの「時刻表」リンクをたどって、静的時刻表を取得しています。
...
ただし、その停留所での行き先が1つしかない系統では、1.で行き先選択ページではなく
**時刻表そのもの**が返ります（選ぶ余地が無いため）。この場合は `func_stoppole` が
含まれないので、2.へは進まず1.のページをそのまま解析します。
...
定刻はウィジェットの見出しに「定刻（土曜ダイヤ）」のようにダイヤ区分を添えて表示します。
表示する便数はスペースに応じて変え、小ウィジェットは2便、中ウィジェットは3便、メニューバーは4便です。
行き先記号（「ｱ」＝途中止まりなど）が付く便は時刻の後ろに記号を付け、メニューバーに
時刻表ページの凡例を出します。ウィジェットは、表示中の定刻に出てくる記号だけを1行で添えます。

本日の運行が終わったあとは、**翌日の始発**を表示します。時刻表ページには平日・土曜・休日の
3つの表がすべて含まれているため、追加の通信なしに翌日分を出せます。ただし翌日がどのダイヤ区分に
なるかはページからは分からないので曜日から推定しており（月〜金→平日、土→土曜、日→休日）、
祝日やお盆などは外れることがあります。そのため見出しを「翌 平日ダイヤ」のように切り替えて、
**どのダイヤの時刻を見せているか**が分かるようにしています。
...
時刻表のページは平日・土曜・休日の3つの表を同時に返すため、1回の取得で全区分が手に入ります。
...
- 接続時にセッションCookie（`JSESSIONID`）が発行されます。事前のログイン等は不要ですが、
  **このCookieを持ち回ってはいけません。** 時刻表は「行き先選択」→「時刻表本体」の2段階で、
  サーバー側がセッションに選択状態を持つため、同じセッションのまま複数系統を並行して取得すると
  互いの選択を上書きし合い、別系統の時刻表が返ります。
  そのため `Shared/BusAPI.swift` は `URLSession.shared` ではなく、
  Cookieを保持しない専用のセッションを使っています
...
- 実車が接近中は「約N分後」の分単位表示、そうでない時間帯は定型文（定刻運行中／まもなく発車／
  接近情報案内不可／運休／接近中のバスなし）をベースに表示します（詳細は「表示される情報について」を参照）
