...
- **時刻セルの分は `Int(cell.text())` で読んではいけない。** 行き先記号が付く便は
  `<td><span>ｱ</span>06</td>` となり、`text()` は「ｱ06」で `Int` が nil になる。
  数字だけを拾うこと。都０５－２ 東京ビッグサイト行きの平日 21〜23 時台はすべて
  「ｱ」（有明一丁目行）で、これを落とすと夜に本日分が空になり翌日ダイヤへ誤切替する。
  業１０の終バス「ﾛ」も同じ構造。記号とページの「記号説明」は捨てず、時刻の後ろと凡例として出す。
  `TimetableParsingTests` が記号付きの分と凡例を固定している
